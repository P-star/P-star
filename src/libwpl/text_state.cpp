...
int wpl_text_state::run_expression(wpl_expression *exp, int index, wpl_value *final_result) {
	if (expression_states[index].get() == nullptr) {
		expression_states[index].reset(exp->new_state(nss));
	}
	return exp->run(expression_states[index].get(), final_result);
}

int wpl_text_state::run_text(wpl_text *text, int index, wpl_value *final_result, ostream &output) {
	if (text_states[index].get() == nullptr) {
		text_states[index].reset(text->new_state(nss));
	}
	return text->run(text_states[index].get(), final_result, output);
}
